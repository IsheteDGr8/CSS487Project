 ...
 *
 * Purpose:
 *   Implements traffic-light detection: TrafficLightFinder plus mask-contour
 *   Hough analysis. Sign classification was moved to ShapeAnalyzer.
 */
...
namespace rsd
{
        ...
        const std::vector<MaskInput> &masks) const
    {
        ...
        for (const MaskInput &input : masks)
        {
            ...
            for (const std::vector<cv::Point> &contour : contours)
            {
                ...
                const Detection detection = labelTrafficLight(input.color, features, contour);
    ...
    Detection RoadObjectDetector::labelTrafficLight(
